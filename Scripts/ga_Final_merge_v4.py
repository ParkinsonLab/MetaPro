...
def merge_fastas(path_0, path_1, section, header, extension, marker_path, bypass_ID_check = True ):
...
def make_merge_leftover_fasta_process(process_store, path_0, path_1, header, tail, marker_location):
    section = ["contigs", "singletons"]
    
    for item in section:
        #merge_fastas(path_0, path_1, item, header, tail, False)
        marker_path = os.path.join(marker_location, item + "_merge_leftover_fasta_" + header)
        merge_process = mp.Process(target = merge_fastas, args = (path_0, path_1, item, header, tail, marker_path, False))
        merge_process.start()
        process_store.append(merge_process)
        #final_fasta_file = os.path.join(path_1, header + "_" + item + tail)
