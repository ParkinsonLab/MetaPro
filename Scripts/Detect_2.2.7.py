...
if __name__=="__main__":
    ...
    #        print( "[DETECT]: Analyzing {} ({}/{}) ...".format(seq.name(), i + 1, len(sequences)))
        ...
        mem_list = psu.virtual_memory()
        mem_percent = mem_list[2]
        print("mem use:", mem_percent)
        mem_limit = 85
        if(process_counter >= n_count or true_job_count == number_of_seqs or mem_percent > mem_limit):
            if(mem_percent > mem_limit):
                print(dt.today(), "processes holding due to mem use limits [" + str(mem_limit) + "%]")
            elif(process_counter >= n_count):
                print(dt.today(), "processes holding due to n-count limit")
            elif(true_job_count == number_of_seqs):
                print(dt.today(), "processes holding due to last-round")
            ...
            final_dict = mp.Manager().dict()    #make a new one that's threadsafe
